...
try
{
    ...
    builder.Services.AddMediatR(cfg => cfg.RegisterServicesFromAssembly(typeof(AiEngineResponseUpdateResponseDisplayedLengthHandler).Assembly));
    builder.Services.AddMediatR(cfg => cfg.RegisterServicesFromAssembly(typeof(ConversationGetAllByUserIdHandler).Assembly));
    builder.Services.AddMediatR(cfg => cfg.RegisterServicesFromAssembly(typeof(UserRequestSaveRequestHandler).Assembly));
